...
class IndiClientTelescope(PyIndi.BaseClient):
    def __init__(self):
        super(IndiClientTelescope, self).__init__()
    def newDevice(self, d):
        name=d.getDeviceName()
        print(f"   newDevice name = {name}")
        pass
...
def getCoordinate(device_telescope):
    ...
    print(f"get Scope coordinates  ra={ra}   dec = {dec}")
    return {'ra':ra,'dec':dec}      

# connect the server
indiclient=IndiClientTelescope()
...
print("connect the scope")
telescope="AstroPhysics GTOCP2"
device_telescope=None
telescope_connect=None
 
print("get the telescope device")
device_telescope=indiclient.getDevice(telescope)
while not(device_telescope):
    time.sleep(0.5)
    device_telescope=indiclient.getDevice(telescope)
     
print("wait CONNECTION property be defined for telescope")
...
getCoordinate(device_telescope)
...
print(f"slew to stars  {starCoords}" )
# We set the desired coordinates
telescope_radec=device_telescope.getNumber("EQUATORIAL_EOD_COORD")
while not(telescope_radec):
    time.sleep(0.5)
    telescope_radec=device_telescope.getNumber("EQUATORIAL_EOD_COORD")
telescope_radec[0].value=starCoords['ra']
telescope_radec[1].value=starCoords['dec']
