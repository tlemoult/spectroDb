...
enum statusExposure { Stop,Running,Finished};


class MyClient : public INDI::BaseClient
{
  public:
    MyClient(char *CCDname,char *pathFileName);
    ~MyClient() = default;

    void setTemperature(int tempSetting);
    void setBinning(int binX,int binY);
    void takeExposure(float duration);
    int  isConnected();
    enum statusExposure status;
    char* myCCD;
    char* savePathFileName;
