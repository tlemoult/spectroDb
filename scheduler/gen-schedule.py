...
for request in requests:
    ...
    ra = request[3].split(' ')
    dec = request[4].split(' ')
    coords = ra[0] + 'h' + ra[1] + 'm' + ra[2] + 's ' + \
        dec[0] + 'd' + dec[1] + 'm' + dec[2] + 's'
    FLUX_V = request[9]
    uid = request[10]
    conf = request[11]
    calib = request[12]
    ...
    if ((request[5] != None) and (request[6] != None)):
        ...
        durationBlock = (ExposureTime * NbExposure +
                         dureeCalibration) * u.second
    else:
        ExposureTime = None
        NbExposure = None
    ...
    if request[8] != None:
        intTime = request[8]  # duree integration pour time serie
    else:
        intTime = None
